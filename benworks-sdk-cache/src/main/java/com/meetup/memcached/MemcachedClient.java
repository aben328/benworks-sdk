...
public class MemcachedClient {
	...
	/**
	 * Enable storing compressed data, provided it meets the threshold requirements. If enabled, data will be stored in
	 * compressed form if it is<br/>
	...
	/**
	 * Sets the required length for data to be considered for compression. If the length of the data to be stored is not
	 * equal or larger than this value, it will not be compressed. This defaults to 15 KB.
	...
	/**
	 * Deletes an object from cache given cache key, a delete time, and an optional hashcode. The item is immediately
	 * made non retrievable.<br/>
	...
	/**
	 * Stores data to cache. If data does not already exist for this key on the server, or if the key is being<br/>
	...
	/**
	 * Increments/decrements the value at the specified key by inc. Note that the server uses a 32-bit unsigned integer,
	 * and checks for<br/>
	...
	/**
	 * Retrieve multiple keys from the memcache. This is recommended over repeated calls to {@link #get(String) get()},
	 * since it<br/>
	 * is more efficient.<br/>
	...
	/**
	 * This method loads the data from cache into a Map. Pass a SockIO object which is ready to receive data and a
	 * HashMap<br/>
	...
	/**
	 * Invalidates the entire cache. Will return true only if succeeds in clearing all servers. If pass in null, then
	 * will try to flush all servers.
	...
	/**
	 * Retrieves stats for all servers. Returns a map keyed on the servername. The value is another map which contains
	 * stats with stat name as key and value as value.
	 * @return Stats map
	 */
	public Map stats() {
		return stats(null);
	...
	/**
	 * Retrieves stats for passed in servers (or all servers). Returns a map keyed on the servername. The value is
	 * another map which contains stats with stat name as key and value as value.
	...
	/**
	 * Retrieves stats items for all servers. Returns a map keyed on the servername. The value is another map which
	...
	/**
	 * Retrieves stats for passed in servers (or all servers). Returns a map keyed on the servername. The value is
	 * another map which contains item stats with itemname:number:field as key and value as value.
	 * @param servers string array of servers to retrieve stats from, or all if this is null
	 * @return Stats map
	 */
	...
	/**
	 * Retrieves stats for passed in servers (or all servers). Returns a map keyed on the servername. The value is
	 * another map which contains slabs stats with slabnumber:field as key and value as value.
	 * @param servers string array of servers to retrieve stats from, or all if this is null
	 * @return Stats map
	 */
	...
	/**
	 * Retrieves items cachedump for all servers. Returns a map keyed on the servername. The value is another map which
	 * contains cachedump stats with the cachekey as key and byte size and unix timestamp as value.
	...
	/**
	 * Retrieves stats for passed in servers (or all servers). Returns a map keyed on the servername. The value is
	 * another map which contains cachedump stats with the cachekey as key and byte size and unix timestamp as value.
	 * @param servers string array of servers to retrieve stats from, or all if this is null
	 * @param slabNumber the item number of the cache dump
	 * @return Stats map
	 */
