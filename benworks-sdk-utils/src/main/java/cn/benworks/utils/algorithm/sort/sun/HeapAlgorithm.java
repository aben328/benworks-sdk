...
/**
 * 第47行有问题 A Heapsort demonstration algorithm HeapAlgorithm.java, 19.04.97
...
class HeapAlgorithm extends SortAlgorithm {
	void sort(int a[]) throws Exception {
		...
		// ----Step 2: Take out elements one by one, largest first, and put them at the bottom
	...
	void perc_down(int ix, int a[], int lng) throws Exception {
		...
		while (i * 2 + 1 <= lng) {
			if (i * 2 + 1 == lng || a[i * 2 + 1] > a[i * 2 + 2]) {
				...
				// if (a[i*2+2]tmp) break; //Position found
				a[i] = a[i * 2 + 2];
				i = i * 2 + 2;
			}
			pause(i, ix);
