 ...
 * <p>
 * 关于这个Cache的介绍，参见
 * http://hi.baidu.com/jabber/blog/item/e6ffa8ec9a36783b27979127.html
 * </p>
 * 
 * @author Matt Tucker
 */
public class DefaultCache<K, V> implements Cache<K, V> {
	...
	private static final Log log = LogFactory.getLog(DefaultCache.class);
	...
	public synchronized V put(K key, V value) {
		...
		DefaultCache.CacheObject<V> cacheObject = new DefaultCache.CacheObject<V>(
				value, objectSize);
	...
	private final class CacheObjectCollection<V> implements Collection<V> {
		...
		private CacheObjectCollection(
				Collection<DefaultCache.CacheObject<V>> cachedObjects) {
		...
		public Iterator<V> iterator() {
			return new Iterator<V>() {
				private final Iterator<DefaultCache.CacheObject<V>> it = cachedObjects
						.iterator();
	...
	public Set<Entry<K, V>> entrySet() {
		...
		synchronized (this) {
			...
			for (final Entry<K, DefaultCache.CacheObject<V>> entry : map
					.entrySet()) {
